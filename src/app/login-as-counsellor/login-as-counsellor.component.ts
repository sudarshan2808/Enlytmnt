...
@Component({
  selector: 'app-login-as-counsellor',
  templateUrl: './login-as-counsellor.component.html',
  styleUrls: ['./login-as-counsellor.component.css']
})
export class LoginAsCounsellorComponent implements OnInit {
  ...
  counsellorImage: any;
  login_url1: string;
  constructor(private activatedRoute:ActivatedRoute,private router:Router, private enlytmntService:EnlytmntService,private appleLoginService:AppleLoginService,private authService: SocialAuthService,private formbuilder:FormBuilder) {
    this.login_url1 = this.activatedRoute.snapshot.queryParamMap.get('login_code');
    this.login_url = decodeURI(atob(this.login_url1));
  this.gerUserImage();
