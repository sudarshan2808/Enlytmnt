...
this.getCountries();
    ...
    this.ReviewData = this.enlytmntService.getApplicationReviewData();
  ...
  ngOnInit(): void {
    this.ApplicationForm = this.formbuilder.group({
      ...
      expertise_area: this.formbuilder.array([this.createExpertise()], [Validators.required]),
...
this.fetchApplicationValue();
  ...
  fetchApplicationValue(){
    this.ApplicationForm.controls['name_prefix'].setValue(this.ReviewData.name_prefix);
    if(this.ReviewData.name_prefix == 'Others'){
    this.otherprefix = true;
    this.ApplicationForm.controls['others'].setValue(this.ReviewData.other_prefix);
    }
    else{
    ...
    this.ApplicationForm.controls['dob'].setValue(this.ReviewData.dob);
    this.ApplicationForm.controls['loc_country'].setValue(this.ReviewData.country);
 ...
 this.educationLength = JSON.parse(this.ReviewData.education);
 ...
 this.educationLength.forEach(element => {
  this.ApplicationForm.controls.Education_more_array.controls[index].controls['university_name'].setValue(element.university);
  this.ApplicationForm.controls.Education_more_array.controls[index].controls['degree'].setValue(element.degree);
  this.ApplicationForm.controls.Education_more_array.controls[index].controls['study_field'].setValue(element.study);
  this.ApplicationForm.controls.Education_more_array.controls[index].controls['year_completion'].setValue(element.year_completion);
 index++;
 });

 //experience
 this.experienceLength = JSON.parse(this.ReviewData.experience);
 ...
 this.experienceLength.forEach(element => {
   this.ApplicationForm.controls.Experience_more_array.controls[index1].controls['job_title'].setValue(element.job_title);
  this.ApplicationForm.controls.Experience_more_array.controls[index1].controls['company_name'].setValue(element.company);
  this.ApplicationForm.controls.Experience_more_array.controls[index1].controls['job_country'].setValue(element.job_country);
  this.ApplicationForm.controls.Experience_more_array.controls[index1].controls['job_state'].setValue(element.job_state);
  this.ApplicationForm.controls.Experience_more_array.controls[index1].controls['job_city'].setValue(element.job_city);
  this.ApplicationForm.controls.Experience_more_array.controls[index1].controls['currently_working'].setValue(element.currently_working);
  this.ApplicationForm.controls.Experience_more_array.controls[index1].controls['experience_from'].setValue(element.experience_from);
  this.ApplicationForm.controls.Experience_more_array.controls[index1].controls['experience_to'].setValue(element.experience_to);
...
this.Imagenamedisplay = this.ReviewData.resumename;
 this.ApplicationForm.controls['uplaod_resume'].setValue(this.ReviewData.resume);
 this.ApplicationForm.controls['referral_code'].setValue(this.ReviewData.referral_code);
  ...
  submitApplication() {
    ...
    this.enlytmntService.CounsellorApplicationApi(formdata).subscribe((res) => {
      if (res.succ) {
        ...
        resumedata.append('extnsn', this.ReviewData.resumeExtn);
        resumedata.append('newfile', this.ReviewData.resume);
  ...
  datechangeValidation(event,field,i){
    ...
    const formvalue = this.ApplicationForm.value;
    var a = new Date();
    var b = new Date(event.target.value);
...
if(Passdate > currentDate){
  ...
  FromTodateValidation(event,field,i){
    ...
    const formvalue = this.ApplicationForm.value;
    var a = new Date();
    var from_date = new Date(formvalue.Experience_more_array[i].experience_from);
    var to_date = new Date(formvalue.Experience_more_array[i].experience_to);
    const currentDate: any = Date.UTC(a.getFullYear(), a.getMonth(), a.getDate());
const fromDate: any = Date.UTC(from_date.getFullYear(), from_date.getMonth(), from_date.getDate());
if(fromDate > currentDate || to_date > currentDate){
  alert("Date should not be more that current date");
  if(field == 'from'){
...
addExpertise() {
  this.expertise_area = this.ApplicationForm.get('expertise_area') as FormArray;
...
// experinece End
  ...
  uploadmulti(event: any) {
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      for (let i = 0; i < filesAmount; i++) {
        ...
        $("#resumeName").text(this.Imagename);
        ...
        reader.onload = (event: any) => {
          ...
          if (this.ImgType[1] == "pdf" || this.ImgType[1] == "doc" || this.ImgType[1] == "docx") {
            ...
            if (this.ImgSize > 2000000) {
              alert("Resume File Size is not greater than 2MB");
  ...
  editReviewForm(){
    this.router.navigate(['/edit_review_application']);
  }
  SelectImage() {
